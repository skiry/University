// MPIBUN.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
inline void send_work(vector <int> &a, vector <int> &b, int nrProcs) {
    ...
    for (int i = 1; i < nrProcs; ++i) {
        ...
        MPI_Send(&st, 1, MPI_INT, i, 1, MPI_COMM_WORLD);
        MPI_Send(&dr, 1, MPI_INT, i, 2, MPI_COMM_WORLD);
        MPI_Send(a.data(), min(dr, n), MPI_INT, i, 3, MPI_COMM_WORLD);
        MPI_Send(b.data(), min(dr, n), MPI_INT, i, 4, MPI_COMM_WORLD);
    }
...
inline void slave(int me) {
    ...
    do_it(st, dr, a, b, res);
    MPI_Send(res.data(), dr - st, MPI_INT, 0, 5, MPI_COMM_WORLD);
