...
class Board:
    ...
    def avMoves(self):
        ...
        dict[6] : random position
        dict[7] : isGameWon: 1=User Win
        '''
        positions = {}
        # 8 positions next to the actual point.
            ...
            for j in range(0,7):
                if self._matrix[i][j] == 0:
                    if 6 not in positions:
                        positions[6] = [(Point(i,j))]
                    else:
                        positions[6].append(Point(i,j))

        if len(self._dict[0]) == 0 and len(self._dict[1]) == 0:
            return positions
    ...
    def okPoint(self,point):
        ...
        if x in range(0,6) and y in range(0,7):
            if self._matrix[x][y]:
                ...
                return False        # it is in range, and not taken
        return 9
