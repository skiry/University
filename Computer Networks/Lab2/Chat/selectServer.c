...
struct sockaddr_in getSocketName(int s, _Bool local_or_remote) {
...
char * getIPAddress(int s, _Bool local_or_remote) {
  struct sockaddr_in addr;
  addr = getSocketName(s, local_or_remote);
  return inet_ntoa(addr.sin_addr);
}

int getPort(int s, _Bool local_or_remote) {
  struct sockaddr_in addr;
  addr = getSocketName(s, local_or_remote);
    ...
    for(;;) {
	  ...
	  if (FD_ISSET(i, &read_fds)) { // we got one!!
	    ...
	    if (i == listener) {
		  ...
		  if ((newfd = accept(listener, (struct sockaddr *)&remoteaddr,(socklen_t*)&addrlen)) == -1) {
		    ...
		    printf("selectserver: new connection from %s on "
			   "socket %d\n", getIPAddress(newfd, 0),newfd);
		   
		    client_count++;
		    sprintf(buf,"Hi-you are client :[%d] (%s:%d) connected to server %s\nThere are %d clients connected\n",
			    newfd, getIPAddress(newfd,0), getPort(newfd, 0),
			    getIPAddress(listener, 1), client_count);
		...
		if ( (strncasecmp("QUIT\n",buf,4) == 0)) {
		  ...
		  nbytes = sprintf(tmpbuf, "<%s - [%d]> %s",getIPAddress(crt,0),crt, buf);
		  sendToALL(tmpbuf, nbytes);
		}
	      }
	    }
